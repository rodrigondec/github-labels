...
class LabelManager(object):
    ...
    def setLabels(self):
        for label in self.labels:
            r = requests.post(self.baseUrl, data=json.dumps(label), auth=self.auth)
...
def main(user, password, repo, organization=None):
    ...
    repo_url = REPO_TEMPLATE_URL.format(user=organization if organization else user, repo=repo)
